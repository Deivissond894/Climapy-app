...
export default function LaunchBudgetScreen() {
  ...
  const processarAudio = async (uri: string) => {
    try {
      ...
      const response = await fetch('https://climapp-1hxc.onrender.com/ai/process-audio', {
        method: 'POST',
  ...
  const uploadImagem = async (uri: string) => {
    try {
      ...
      const response = await fetch(
        'https://climapp-1hxc.onrender.com/upload/orcamento',
        {
          method: 'POST',
          body: formData,
          headers: {
  ...
  const removerImagem = async (index: number, url: string) => {
    try {
      ...
      try {
        const response = await fetch(
          `https://climapp-1hxc.onrender.com/upload/orcamento/${publicId}`,
          { method: 'DELETE' }
        );
  ...
  const salvarOrcamento = async () => {
    try {
      ...
      console.log('URL do endpoint:', `https://climapp-1hxc.onrender.com/atendimentos/${params.atendimentoId}/orcamento`);
      console.log('atendimentoId recebido:', params.atendimentoId);

      const response = await fetch(
        `https://climapp-1hxc.onrender.com/atendimentos/${params.atendimentoId}/orcamento`,
        {
          method: 'PUT',
          headers: {
  ...
  const consultarPrecos = async () => {
    try {
      ...
      const response = await fetch(
        `https://climapp-1hxc.onrender.com/atendimentos/${user?.id}/${params.atendimentoId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
